...
# Fix 4: Deepgram returns synthesized audio bytes from /v1/speak, but the
#         upstream OpenAPI currently labels the successful response as JSON
#         with an empty object schema. Preserve the binary response contract so
#         AutoSDK emits GenerateAsStreamAsync for incremental playback.
yq -i '
  .paths."/v1/speak".post.responses."200".content = {
...
# Fix 5: Upstream currently publishes an AsyncAPI 2.6 document with
...
# Fix 6: AsyncAPI spec has two security schemes (ApiKeyAuth + JwtAuth) that both
